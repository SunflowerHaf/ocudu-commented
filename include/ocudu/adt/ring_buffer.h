...
// =============================================================================
// FILE: include/ocudu/adt/ring_buffer.h  (520 lines)
//
...
public:
  ...
  void set_size(unsigned sz)
  {
    ocudu_assert(count == 0 or sz == data.size(), "Dynamic resizes not supported when ring buffer is not empty");
    data.resize(sz);
  }
...
class ring_buffer_impl
{
  ...
  class iterator_impl
  {
    ...
    {
      ocudu_assert(ring_idx <= parent->size(), "Invalid iterator position");
    }
  ...
  private:
    ...
    {
      ocudu_assert(ring_idx < parent->size(),
    ...
    void inc_()
    {
      ocudu_assert(*this != parent->end(), "Incrementing iterator beyond end()");
      ring_idx++;
    }
    ...
    {
      if (n > 0) {
        ocudu_assert(ring_idx + n <= parent->size(), "Incrementing iterator beyond end()");
      } else if (n < 0) {
        ocudu_assert(ring_idx + n <= parent->size(), "Decrementing iterator beyond begin()");
      }
      ring_idx += n;
    }
...
public:
  ...
  {
    ocudu_assert(not full(), "Circular buffer is full.");
  ...
  void pop()
  {
    ocudu_assert(not empty(), "Cannot call pop() in empty circular buffer");
  ...
  T& top()
  {
    ocudu_assert(not empty(), "Cannot call top() in empty circular buffer");
  ...
  {
    ocudu_assert(i < size(), "Out-of-bounds access to circular buffer ({} >= {})", i, buffer.count);
