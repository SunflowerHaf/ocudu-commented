...
// =============================================================================
// FILE: include/ocudu/support/async/async_procedure.h  (125 lines)
//
...
struct base_resumable_procedure {
  ...
  virtual ~base_resumable_procedure() = default;
  ...
  void operator()(detail::base_coro_frame<promise_type>& ctx)
  {
    if (frame_ptr == nullptr) {
      ocudu_sanity_check(ctx.state_index == detail::tag_init, "Invalid coro state");
      ctx.state_index = 10;
      frame_ptr       = &ctx;
  ...
  void async_await(Awaitable&& a, void (Derived::*action)(Args...))
  {
    auto resume_func = [this, action]() {
      if (this->frame_ptr->state_index > 0) {
        ...
        ocudu_sanity_check(frame_ptr->state_index != detail::coro_state_tag_t::tag_cancelled,
                           "Calling resume on cancelled task");
        ocudu_sanity_check(frame_ptr->state_index != detail::coro_state_tag_t::tag_final_suspend,
...
private:
  ...
  /// Pointer to coroutine frame.
  detail::base_coro_frame<promise_type>* frame_ptr = nullptr;
  /// Next method to call on resume.
  /// Next method to call on resume.
  std::function<void()> resume_method;
};
