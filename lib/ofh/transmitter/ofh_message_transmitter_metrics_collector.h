...
// =============================================================================
// FILE: lib/ofh/transmitter/ofh_message_transmitter_metrics_collector.h  (81 lines)
//
...
/// Open Fronthaul message transmitter metrics collector.
class message_transmitter_metrics_collector
{
public:
  message_transmitter_metrics_collector(bool is_enabled) : is_disabled(!is_enabled) {}
  ...
  /// Updates the statistics given the execution time taken by the message transmitter implementation.
  ...
  /// Collects message transmitter metrics.

  /// Collects message transmitter metrics.
  void collect_metrics(message_transmitter_metrics& metrics)
  {
    if (is_disabled) {
      return;
    }
    ...
    metrics.message_tx_avg_latency_us = count_val ? static_cast<double>(sum_elapsed_val_ns) / count_val / 1000.0 : 0.f;


    metrics.message_tx_min_latency_us =
        (min_latency_val_ns == default_min_latency_ns) ? 0.f : static_cast<float>(min_latency_val_ns) / 1000.f;


    metrics.message_tx_max_latency_us =
