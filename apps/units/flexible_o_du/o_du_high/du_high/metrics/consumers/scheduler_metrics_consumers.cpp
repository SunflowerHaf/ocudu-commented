...
// =============================================================================
// FILE: metrics/consumers/scheduler_metrics_consumers.cpp
//
// Purpose (plain English):
//   Implements the three scheduler metric consumers (stdout, log, E2).
//
//   STDOUT consumer:
//     Prints a compact table where each row is one UE currently connected.
//     Columns: PCI  RNTI | CQI  RI  MCS  brate  ok  nok  (%)  dl_bs |
//              pusch_snr  rsrp  ri  mcs  brate  ok  nok  (%)  bsr  ta  phr
//     A header line is reprinted after every 10 rows, or on request.
//
//   Log consumer:
//     Writes two kinds of log entries per metrics period:
//       * A cell-level summary: PRB utilisation, grant counts, decision
//         latency histogram, failed PDCCH allocations, PRACH delays, etc.
//       * One per-UE entry: all columns from the stdout table plus HARQ
//         failure details, OL-LA parameters, CRC/CE/PUSCH/PUCCH delays.
//     This is very verbose but contains everything needed to diagnose
//     scheduler problems without live console access.
//
//   E2 consumer:
//     One call to notifier.report_metrics(cell) per cell per period. The
//     E2 agent maps this into an E2SM-KPM RIC Indication message.
// =============================================================================
...
// Prints the two-line column header for the stdout table.
...
// Maps scheduler event type enum to a human-readable string.
...
void scheduler_cell_metrics_consumer_stdout::handle_metric(const std::optional<scheduler_metrics_report>& report)
{
  ...
  for (const auto& cell : report->cells) {
    ...
    // Reprint header if too many rows have accumulated without one.
    ...
    for (const auto& ue : cell.ue_metrics) {
      ...
      // CQI (Channel Quality Indicator): shows what MCS range is achievable.
      ...
      // DL MCS, bitrate, success/failure counts and error rate, buffer status.
      ...
      // UL section: PUSCH SNR, RSRP, RI, MCS, bitrate, ok/nok/%, BSR, TA, PHR.
      ...
      // BSR (Buffer Status Report): how much data the UE wants to send (bytes).
      // TA (Timing Advance) in seconds, engineering notation.
      } else {
        fmt::print("    n/a");
      }
      // PHR (Power Headroom Report): how much uplink power margin the UE has (dB).
...
void scheduler_cell_metrics_consumer_log::handle_metric(const std::optional<scheduler_metrics_report>& report)
{
  ...
  for (const auto& cell : report->cells) {
    ...
    // Compute max per-UE delays to append if non-trivial.
    ...
    // Compute total PRBs used.
    }

    // log cell-wide metrics

    // Cell-level summary line.
    fmt::format_to(
    ...
    // Per-TDD-slot PRB usage (only logged if the cell is TDD).
    ...
    // Worst-case delays across all UEs in this period.
    ...
    // Scheduler events this period (UE add/remove/reconfigure).
    ...
    // Per-UE log entries: full DL+UL breakdown.
    for (const auto& ue : cell.ue_metrics) {
      fmt::format_to(std::back_inserter(buffer),
      ...
      // TA in seconds, SRS-TA, PHR, and worst-case scheduling distances.
...
void scheduler_cell_metrics_consumer_e2::handle_metric(const std::optional<scheduler_metrics_report>& report)
{
  ...
  // One call per cell; the E2 agent handles multi-cell internally.
  for (const auto& cell : report->cells) {
