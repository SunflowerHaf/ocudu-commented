// SPDX-FileCopyrightText: Copyright (C) 2021-2026 Software Radio Systems Limited
// SPDX-License-Identifier: BSD-3-Clause-Open-MPI

// SPDX-FileCopyrightText: Copyright (C) 2021-2026 Software Radio Systems Limited
// SPDX-License-Identifier: BSD-3-Clause-Open-MPI
// Portions of this file may implement 3GPP specifications, which may be subject to additional licensing requirements.

// =============================================================================
// FILE: lib/ocuduvec/subtract.cpp  (136 lines)
//
// LIBRARY: lib/ocuduvec
// Custom SIMD-accelerated vector types and compile-time utilities. Provides ocuduvec types (aligned buffers, SIMD intrinsic wrappers) used throughout the PHY for DSP operations (FFT, channel estimation, modulation). Eliminates dependency on external SIMD libraries for basic operations.
//
// This file is part of the above library. See library comment for context.
// =============================================================================

#include "ocudu/ocuduvec/subtract.h"
#include "ocudu/ocuduvec/simd.h"


using namespace ocudu;
using namespace ocuduvec;


static void subtract_fff_simd(float* z, const float* x, const float* y, std::size_t len)
{
  std::size_t i = 0;


#if OCUDU_SIMD_F_SIZE
  if (SIMD_IS_ALIGNED(x) && SIMD_IS_ALIGNED(y) && SIMD_IS_ALIGNED(z)) {
    for (std::size_t i_end = (len / OCUDU_SIMD_F_SIZE) * OCUDU_SIMD_F_SIZE; i != i_end; i += OCUDU_SIMD_F_SIZE) {
      simd_f_t a = ocudu_simd_f_load(&x[i]);
      simd_f_t b = ocudu_simd_f_load(&y[i]);


      simd_f_t r = ocudu_simd_f_sub(a, b);


      ocudu_simd_f_store(&z[i], r);
    }
  } else {
    for (std::size_t i_end = (len / OCUDU_SIMD_F_SIZE) * OCUDU_SIMD_F_SIZE; i != i_end; i += OCUDU_SIMD_F_SIZE) {
      simd_f_t a = ocudu_simd_f_loadu(&x[i]);
      simd_f_t b = ocudu_simd_f_loadu(&y[i]);


      simd_f_t r = ocudu_simd_f_sub(a, b);


      ocudu_simd_f_storeu(&z[i], r);
    }
  }
#endif


  for (; i != len; ++i) {
    z[i] = x[i] - y[i];
  }
}


static void subtract_sss_simd(int16_t* z, const int16_t* x, const int16_t* y, std::size_t len)
{
  std::size_t i = 0;


#if OCUDU_SIMD_S_SIZE
  if (SIMD_IS_ALIGNED(x) && SIMD_IS_ALIGNED(y) && SIMD_IS_ALIGNED(z)) {
    for (std::size_t i_end = (len / OCUDU_SIMD_S_SIZE) * OCUDU_SIMD_S_SIZE; i != i_end; i += OCUDU_SIMD_S_SIZE) {
      simd_s_t a = ocudu_simd_s_load(&x[i]);
      simd_s_t b = ocudu_simd_s_load(&y[i]);


      simd_s_t r = ocudu_simd_s_sub(a, b);


      ocudu_simd_s_store(&z[i], r);
    }
  } else {
    for (std::size_t i_end = (len / OCUDU_SIMD_S_SIZE) * OCUDU_SIMD_S_SIZE; i != i_end; i += OCUDU_SIMD_S_SIZE) {
      simd_s_t a = ocudu_simd_s_loadu(&x[i]);
      simd_s_t b = ocudu_simd_s_loadu(&y[i]);


      simd_s_t r = ocudu_simd_s_sub(a, b);


      ocudu_simd_s_storeu(&z[i], r);
    }
  }
#endif /* OCUDU_SIMD_S_SIZE */


  for (; i != len; ++i) {
    z[i] = x[i] - y[i];
  }
}


static void subtract_bbb_simd(int8_t* z, const int8_t* x, const int8_t* y, std::size_t len)
{
  std::size_t i = 0;


#if OCUDU_SIMD_S_SIZE
  if (SIMD_IS_ALIGNED(x) && SIMD_IS_ALIGNED(y) && SIMD_IS_ALIGNED(z)) {
    for (std::size_t i_end = (len / OCUDU_SIMD_B_SIZE) * OCUDU_SIMD_B_SIZE; i != i_end; i += OCUDU_SIMD_B_SIZE) {
      simd_b_t a = ocudu_simd_b_load(&x[i]);
      simd_b_t b = ocudu_simd_b_load(&y[i]);


      simd_b_t r = ocudu_simd_b_sub(a, b);


      ocudu_simd_b_store(&z[i], r);
    }
  } else {
    for (std::size_t i_end = (len / OCUDU_SIMD_B_SIZE) * OCUDU_SIMD_B_SIZE; i != i_end; i += OCUDU_SIMD_B_SIZE) {
      simd_b_t a = ocudu_simd_b_loadu(&x[i]);
      simd_b_t b = ocudu_simd_b_loadu(&y[i]);


      simd_b_t r = ocudu_simd_b_sub(a, b);


      ocudu_simd_b_storeu(&z[i], r);
    }
  }
#endif /* OCUDU_SIMD_S_SIZE */


  for (; i != len; ++i) {
    z[i] = x[i] - y[i];
  }
}


void ocudu::ocuduvec::subtract(span<cf_t> z, span<const cf_t> x, span<const cf_t> y)
{
  ocudu_ocuduvec_assert_size(x, y);
  ocudu_ocuduvec_assert_size(x, z);


  subtract_fff_simd(reinterpret_cast<float*>(z.data()),
                    reinterpret_cast<const float*>(x.data()),
                    reinterpret_cast<const float*>(y.data()),
                    2 * z.size());
}


void ocudu::ocuduvec::subtract(span<float> z, span<const float> x, span<const float> y)
{
  ocudu_ocuduvec_assert_size(x, y);
  ocudu_ocuduvec_assert_size(x, z);


  subtract_fff_simd(z.data(), x.data(), y.data(), z.size());
}


void ocudu::ocuduvec::subtract(span<int16_t> z, span<const int16_t> x, span<const int16_t> y)
{
  ocudu_ocuduvec_assert_size(x, y);
  ocudu_ocuduvec_assert_size(x, z);


  subtract_sss_simd(z.data(), x.data(), y.data(), z.size());
}


void ocudu::ocuduvec::subtract(span<int8_t> z, span<const int8_t> x, span<const int8_t> y)
{
  ocudu_ocuduvec_assert_size(x, y);
  ocudu_ocuduvec_assert_size(x, z);


  subtract_bbb_simd(z.data(), x.data(), y.data(), z.size());
}
