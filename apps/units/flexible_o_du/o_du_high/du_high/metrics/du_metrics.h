...
// =============================================================================
// FILE: metrics/du_metrics.h
//
// Purpose (plain English):
//   Parallel to du_high_rlc_metrics.h but for DU-level combined metrics
//   (scheduler + MAC). The odu::du_metrics_report aggregates both the
//   scheduler per-UE table and the MAC downlink timing statistics into one
//   snapshot. Consumers can pick the fields they care about.
// =============================================================================
...
/// Provides the name "DU metrics" to the framework's reporting.
...
/// \brief A single DU metrics snapshot wrapping odu::du_metrics_report.
///
/// Contains both the scheduler per-UE table (dl_mcs, ul_mcs, brate, HARQ,
/// SRS reports) and the MAC timing latency breakdown (wall-clock latency,
/// sched latency, DL/UL TTI request latencies, context switch counts).
class du_metrics_impl : public app_services::metrics_set
{
  du_metrics_properties_impl properties;
  odu::du_metrics_report     report; ///< The combined scheduler + MAC data.
...
/// Dispatch callback: defers consumer calls onto the metrics executor.
