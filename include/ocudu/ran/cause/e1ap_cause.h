...
// =============================================================================
// FILE: include/ocudu/ran/cause/e1ap_cause.h  (87 lines)
//
...
// e1ap_cause_t formatter
template <>
struct formatter<ocudu::e1ap_cause_t> {
  ...
  auto format(ocudu::e1ap_cause_t o, FormatContext& ctx) const
  {
    if (const auto* result = std::get_if<ocudu::e1ap_cause_radio_network_t>(&o)) {
      return format_to(ctx.out(), "radio_network-id{}", fmt::underlying(*result));
    }
    if (const auto* result = std::get_if<ocudu::e1ap_cause_transport_t>(&o)) {
