...
// =============================================================================
// FILE: include/ocudu/ocuduvec/convolution.h  (114 lines)
//
...
void convolution_same(V&& out, const T& x_v, const U& y_v)
{
  ...
  static_assert((is_arithmetic_span_compatible<V>::value || is_complex_span_compatible<V>::value),
                "Template type is not compatible with a span of arithmetics or complex floats.");
  ocudu_ocuduvec_assert_size(out, x_v);


  unsigned x_size = x_v.size();
  unsigned y_size = y_v.size();
  ocudu_assert((y_size <= x_size),
  ...
  ocudu_assert(detail::check_different(out, x_v), "Cannot override input with output.");
