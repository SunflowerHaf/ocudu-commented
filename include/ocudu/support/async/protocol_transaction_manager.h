...
// =============================================================================
// FILE: include/ocudu/support/async/protocol_transaction_manager.h  (420 lines)
//
...
public:
  ...
  bool complete() const
  {
    ocudu_assert(valid(), "Trying to check completion of invalid transaction");
    return ev->is_set();
  }
  ...
  {
    ocudu_assert(aborted(), "Trying to get result of invalid transaction");
    return ev->get().error();
  }
  ...
  {
    ocudu_assert(has_response(), "Trying to get response of transaction without a response");
    return ev->get().value();
  }
  response_type response() &&
  {
    ocudu_assert(has_response(), "Trying to get result of transaction without a response");
  ...
  {
    ocudu_assert(complete(), "Trying to fetch outcome of incomplete transaction");
    return ev->get();
  }
  ...
  {
    ocudu_assert(valid(), "Trying to fetch awaiter of invalid transaction");
...
public:
  ...
  ~protocol_transaction_manager()
  {
    if (not running_transactions.empty()) {
      ocudulog::fetch_basic_logger("ALL").error("Protocol transaction manager destroyed with {} running transactions",
  ...
  [[nodiscard]] protocol_transaction<T> create_transaction(std::chrono::milliseconds time_to_cancel = max_timeout)
  {
    ...
    if (transaction_id == invalid_protocol_transaction_id) {
      ocudulog::fetch_basic_logger("ALL").error("Failed to allocate new transaction id");
      return {};
    }
                                                           ...
                                                           std::chrono::milliseconds time_to_cancel = max_timeout)
  {
    ...
    if (not ret.second) {
      ocudulog::fetch_basic_logger("ALL").error("Failed to allocate transaction id={}", transaction_id);
      return {};
    }
...
private:
  ...
  void remove_transaction(protocol_transaction_id_t transaction_id)
  {
    auto it = running_transactions.find(transaction_id);
    if (it == running_transactions.end()) {
      ocudulog::fetch_basic_logger("ALL").warning("Trying to remove non-existing transaction id={}", transaction_id);
      return;
    }
...
public:
  ...
  {
    ocudu_assert(successful(), "Trying to fetch incorrect transaction result");
    return std::get<success_response_type>(observer.result());
  }
  const failure_response_type& failure() const
  {
    ocudu_assert(failed(), "Trying to fetch incorrect transaction result");
