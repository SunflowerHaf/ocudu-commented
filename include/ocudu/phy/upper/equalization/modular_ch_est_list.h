...
// =============================================================================
// FILE: include/ocudu/phy/upper/equalization/modular_ch_est_list.h  (104 lines)
//
...
public:
  ...
  {
    ocudu_assert(buffer.size() == nof_re,
  ...
  {
    ocudu_assert(nof_rx_ports * nof_layers <= MaxNofElements,
  ...
  {
    ocudu_assert(i_rx_port < data.get_dimension_size(ch_dims::rx_port),
                 "The receive port index (i.e., {}) exceeds the number of receive ports (i.e., {}).",
                 i_rx_port,
                 data.get_dimension_size(ch_dims::rx_port));
    ocudu_assert(i_layer < data.get_dimension_size(ch_dims::tx_layer),
