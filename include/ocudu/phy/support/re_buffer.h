...
// =============================================================================
// FILE: include/ocudu/phy/support/re_buffer.h  (451 lines)
//
...
public:
  ...
  {
    ocudu_assert((buffer.get_nof_re() >= offset + count),
...
public:
  ...
  {
    ocudu_assert(nof_slices <= max_nof_slices,
                 "The number of slices (i.e., {}) exceeds the maximum (i.e., {}).",
                 nof_slices,
                 max_nof_slices);
    ocudu_assert(nof_re <= max_nof_re,
...
public:
  ...
  void resize(unsigned nof_slices, unsigned nof_re)
  {
    ocudu_assert(nof_slices <= MaxNofSlices,
                 "The number of slices (i.e., {}) exceeds the maximum (i.e., {}).",
                 nof_slices,
                 MaxNofSlices);
    ocudu_assert(nof_re <= MaxNofRe,
                 "The number of resource elements (i.e., {}) exceeds the maximum (i.e., {}).",
                 nof_re,
                 MaxNofRe);
    data.resize({nof_re, nof_slices});
...
public:
  ...
  void resize(unsigned nof_slices_, unsigned nof_re_)
  {
    ...
    nof_re     = nof_re_;
    ocudu_assert(nof_slices <= data.size(),
                 "The number of slices (i.e., {}) exceeds the maximum (i.e., {}).",
                 nof_slices,
                 data.size());
  ...
  span<const T> get_slice(unsigned i_slice) const override
  {
    ocudu_assert(i_slice < nof_slices,
                 "The slice index (i.e., {}) exceeds the number of slices (i.e., {}).",
                 i_slice,
                 nof_slices);
    ocudu_assert(!data[i_slice].empty(), "Data for slice {} is empty.", i_slice);
    return data[i_slice];
  }
...
public:
  ...
  {
    ocudu_assert(offset + nof_re_ <= buffer.get_nof_re(),
