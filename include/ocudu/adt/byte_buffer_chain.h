...
// =============================================================================
// FILE: include/ocudu/adt/byte_buffer_chain.h  (394 lines)
//
...
class byte_buffer_chain
{
  ...
  public:
    ...
    {
      ocudu_assert(buffer_it != parent->slices().end(), "Out-of-bound access");
      ++it;
      if (it == buffer_it->end()) {
        ++buffer_it;
...
public:
  ...
  const uint8_t& operator[](size_t idx) const
  {
    ...
    ocudu_assertion_failure("Out-of-bounds access ({} >= {})", idx, length());
