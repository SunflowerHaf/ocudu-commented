...
// =============================================================================
// FILE: include/ocudu/adt/detail/byte_buffer_segment.h  (171 lines)
//
...
public:
  ...
  uint8_t& back()
  {
    ocudu_assert(not empty(), "back() called for empty segment.");
    return payload.back();
  }
  ...
  {
    ocudu_assert(idx < length(), "Out-of-bound access");
    return *(begin() + idx);
  }
  ...
  {
    ocudu_assert(nof_bytes <= capacity() - headroom(), "There is not enough space for provided size");
  ...
  void append(It it_begin, It it_end)
  {
    ...
    ocudu_assert((size_t)(it_end - it_begin) <= tailroom(), "There is not enough tailroom for append.");
  ...
  {
    ocudu_assert(tailroom() >= 1, "There is not enough tailroom space.");
  ...
  {
    ocudu_assert(headroom() >= bytes.size(), "There is not enough headroom space.");
  ...
  {
    ocudu_assert(headroom() >= nof_bytes, "There is not enough headroom space.");
  ...
  {
    ocudu_assert(nof_bytes <= length(), "There is not enough headroom space.");
    payload = span<uint8_t>{payload.begin() + nof_bytes, payload.end()};
  }
