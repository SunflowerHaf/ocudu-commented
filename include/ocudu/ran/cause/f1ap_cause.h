...
// =============================================================================
// FILE: include/ocudu/ran/cause/f1ap_cause.h  (99 lines)
//
...
// f1ap_cause_t formatter
template <>
struct formatter<ocudu::f1ap_cause_t> {
  ...
  auto format(ocudu::f1ap_cause_t o, FormatContext& ctx) const
  {
    if (const auto* cause = std::get_if<ocudu::f1ap_cause_radio_network_t>(&o)) {
      return format_to(ctx.out(), "radio_network-id{}", fmt::underlying(*cause));
    }
    if (const auto* cause = std::get_if<ocudu::f1ap_cause_transport_t>(&o)) {
