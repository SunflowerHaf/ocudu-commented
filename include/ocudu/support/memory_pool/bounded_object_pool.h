...
// =============================================================================
// FILE: include/ocudu/support/memory_pool/bounded_object_pool.h  (589 lines)
//
...
public:
  ...
  struct base_segment {
    ...
    {
      ocudu_assert(size() > 0 and size() <= max_size(), "Number of objects exceeds segment capacity");
  ...
  {
    ocudu_assert(seg_idx < segments.size(), "Invalid segment index");


    SegmentType& seg  = *segments[seg_idx];
    uint64_t     prev = seg.busy_bitmap.fetch_and(~(1ULL << obj_idx), std::memory_order_release);
    ocudu_assert(prev & (1ULL << obj_idx), "Reclaiming an object that was not allocated");
  }
...
public:
  ...
  ptr get()
  {
    ...
    ocudu_sanity_check(obj_idx < impl.segments[seg_idx]->objects.size(), "Invalid object index");
