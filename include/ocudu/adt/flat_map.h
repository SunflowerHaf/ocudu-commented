...
// =============================================================================
// FILE: include/ocudu/adt/flat_map.h  (504 lines)
//
...
class flat_map : private detail::flat_compare<Compare>
{
  ...
  public:
    ...
    {
      ocudu_assert(lhs.parent == rhs.parent, "Subtracting iterators from different containers");
      return lhs.index - rhs.index;
    }
    friend bool operator==(const iter_impl& lhs, const iter_impl& rhs) noexcept
    {
      ocudu_assert(lhs.parent == rhs.parent, "Comparing iterators from different containers");
      return lhs.index == rhs.index;
    }
...
public:
  ...
  {
    ocudu_assert(conts.keys.size() == conts.values.size(), "Key and value containers must have the same size");
