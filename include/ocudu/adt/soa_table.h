...
// =============================================================================
// FILE: include/ocudu/adt/soa_table.h  (447 lines)
//
...
public:
  ...
  auto& at()
  {
    ...
    ocudu_sanity_check(offset < cols.size(), "Row offset out of range");
    return cols[offset];
  }
  template <auto ColId>
  const auto& at() const
...
public:
  ...
  row_id insert(Vals&&... vals)
  {
    ...
    if (rid < index_map.size()) {
      ...
      ocudu_sanity_check(rid == index_map.size(), "Invalid free_list_head");
  ...
  bool erase(row_id rid)
  {
    ...
    ocudu_sanity_check(index_reverse_map[offset] == rid, "Invalid row_id");
...
struct std::tuple_element<ColIndex, ocudu::soa::row_view<Table>> {
  using type = typename Table::template column_type<ColIndex>;
};


template <typename Table>
struct std::tuple_size<ocudu::soa::row_view<Table>> : std::integral_constant<std::size_t, Table::nof_columns()> {};
