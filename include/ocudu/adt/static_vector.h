...
// =============================================================================
// FILE: include/ocudu/adt/static_vector.h  (465 lines)
//
...
struct trivial_storage {
  ...
  {
    ocudu_assert(i < sz, "out-of-bounds access ({} >= {})", i, sz);
    return array[i];
  }
  ...
  {
    ocudu_assert(sz != 0, "back() called for empty vector");
    return array[sz - 1];
  }
...
struct non_trivial_storage {
  ...
  {
    ocudu_assert(i < Capacity, "out-of-bounds access ({} >= {})", i, sz);
    return *(data() + i);
  }
...
public:
  ...
  explicit constexpr static_vector(size_type count) noexcept(std::is_nothrow_default_constructible_v<T>)
  {
    ...
    ocudu_assert(count <= MAX_N, "static vector maximum size={} was exceeded", MAX_N);
    // Note: Gcc 11 fails compilation without this hint. Potentially a bug.
    // Note: Gcc 11 fails compilation without this hint. Potentially a bug.
    ocudu_assume(count <= MAX_N);
  ...
  {
    ocudu_assert(size() < MAX_N, "Cannot emplace back in full vector");
  ...
  {
    ocudu_assert(not empty(), "Trying to erase element from empty vector");
    this->destroy_(&back());
    this->sz--;
  }
  ...
  {
    static_assert(std::is_default_constructible<T>::value, "T must be default constructible");
    ocudu_assert(new_size <= capacity(), "out-of-bounds construction in static_vector<T,N>");
    if (new_size <= size()) {
      // vector is shrinking.
      // vector is shrinking.
  ...
  constexpr iterator emplace(const_iterator pos, Args&&... args) noexcept(std::is_nothrow_move_assignable_v<T>)
  {
    ...
    ocudu_assert(this->sz < MAX_N, "Cannot insert in full vector");
  ...
  {
    ocudu_assert(pos >= this->begin() and pos < this->end(), "Iterator to erase is out-of-bounds");
    iterator ret = pos;
  ...
  {
    ocudu_assert(it_start <= it_end, "Trying to erase invalid range");
    ocudu_assert(it_start >= begin() and it_end <= end(), "Range to erase is out-of-bounds");
...
private:
  ...
  {
    ocudu_assert(N <= MAX_N, "static vector maximum size={} was exceeded", MAX_N);
    // Note: Gcc 11 fails compilation without this hint. Potentially a bug.
    // Note: Gcc 11 fails compilation without this hint. Potentially a bug.
    ocudu_assume(N <= MAX_N);
