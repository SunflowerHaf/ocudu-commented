...
// =============================================================================
// FILE: lib/ocudulog/formatters/text_formatter.cpp  (211 lines)
//
...
{
  fmt::format_to(std::back_inserter(buffer), "### Potentially skipped log lines from this point onwards ###\n");
}
  ...
  if (metadata.fmtstring) {
    if (metadata.store) {
      ...
      try {
        ...
        fmt::print(stderr, "ocuduLog error - Invalid format string: \"{}\"\n", metadata.fmtstring);
        fmt::format_to(
            std::back_inserter(buffer), " -> ocuduLog error - Invalid format string: \"{}\"", metadata.fmtstring);
                                        ...
                                        fmt::memory_buffer&               buffer)
{
  ...
  if (md.store) {
    ...
    try {
      ...
      fmt::print(stderr, "ocuduLog error - Invalid format string: \"{}\"\n", md.fmtstring);
      fmt::format_to(std::back_inserter(buffer), " -> ocuduLog error - Invalid format string: \"{}\"", md.fmtstring);
#ifdef STOP_ON_WARNING
      std::abort();
#endif
    }
