...
// =============================================================================
// FILE: services/metrics/metrics_manager.h
//
// Purpose (plain English):
//   The central metrics fan-out hub. Implements metrics_notifier — producers call on_new_metric() when a snapshot is ready. The manager looks up which metrics_entry matches the metric name, then calls its dispatch callback (which runs the consumers on a dedicated task executor, off the real-time path). Owns a periodic_metrics_report_controller that fires once per reporting period to call on_new_report_period() on all producers.
// =============================================================================
