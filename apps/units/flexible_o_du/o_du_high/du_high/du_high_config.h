...
// =============================================================================
// FILE: du_high_config.h
//
// Purpose (plain English):
//   The master configuration data model for the DU High ("upper" half of the
//   Distributed Unit). This is the largest config file in the application
//   layer — 1400+ lines — because the DU High is responsible for everything
//   from MAC scheduling down to the PDCP layer, meaning it has an enormous
//   number of tuneable parameters drawn from 3GPP TS 38.211/212/213/214/321/
//   322/323/331.
//
//   STRUCT MAP (reading order):
//
//   Logging & tracing
//     du_high_unit_logger_config    — per-channel log levels (DU, MAC, RLC, F1AP, F1-U, GTPU, NTN)
//     du_high_unit_tracer_config    — executor-tracing flag
//
//   Scheduler
//     du_high_unit_ta_sched_control_config — timing-advance closed-loop control
//     du_high_unit_scheduler_config        — expert scheduler parameters
//     du_high_unit_drx_config              — discontinuous reception (power saving)
//
//   Physical-layer cell config (shared by all PHY-adjacent layers)
//     du_high_unit_ssb_config       — SSB (Synchronisation Signal Block) power & period
//     du_high_unit_ul_common_config — UL maximum HARQ retransmissions, p0-NominalWithGrant
//     du_high_unit_pdsch_config     — DL shared channel: MCS table, TBS limits, DMRS
//     du_high_unit_pusch_config     — UL shared channel: MCS table, DMRS, DMRS positions,
//                                     beta offsets, power control, transform precoding
//     du_high_unit_pucch_config     — UL control channel: format selection, resource sets
//     du_high_unit_srs_config       — Sounding Reference Signals (for channel estimation)
//     du_high_unit_phy_cell_group_config — HARQ-ACK codebook type
//
//   Frame structure
//     tdd_ul_dl_pattern_unit_config — one TDD DL/UL pattern (period, DL/UL slots)
//     du_high_unit_tdd_ul_dl_config — primary + optional secondary TDD pattern
//
//   Paging & PDCCH
//     du_high_unit_paging_config    — paging parameters (DRX cycle, paging frame)
//     pdcch_common_unit_config      — common PDCCH (CSS) parameters
//     pdcch_dedicated_unit_config   — UE-specific PDCCH (USS) parameters
//     du_high_unit_pdcch_config     — aggregates common + dedicated
//
//   Neighbour cells & SIB content
//     pci_range_config              — a PCI range for excluded-cell lists
//     du_high_unit_sib_config       — SIB1/2/3/4/5/19 content (neighbour cells,
//                                     cell reselection, NTN, UAC, etc.)
//
//   CSI & MAC sub-layer
//     du_high_unit_csi_config       — CSI-RS reporting configuration
//     mac_bsr_unit_config           — Buffer Status Report timers
//     mac_phr_unit_config           — Power Headroom Report triggers
//     mac_sr_unit_config            — Scheduling Request configuration
//     du_high_unit_mac_cell_group_config — aggregates BSR + PHR + SR
//
//   RACH (Random Access)
//     du_high_unit_rach_config      — preamble format, ZCZ, power, back-off
//
//   Network slicing
//     du_high_unit_cell_slice_sched_config — per-slice PRB allocation ratios
//     du_high_unit_cell_slice_config       — slice identity + scheduler params
//
//   Radio-link monitoring
//     du_high_unit_rlm_config       — beam failure thresholds
//
//   Cell aggregation
//     du_high_unit_base_cell_config — all per-cell parameters in one struct
//     du_high_unit_cell_config      — base cell + optional NTN config
//
//   GNB identity, metrics, PCAPs, execution
//     du_high_unit_metrics_layer_config  — per-layer metric enable flags
//     du_high_unit_metrics_config        — aggregates layers + common
//     du_high_unit_pcap_config           — F1AP, F1-U, E2AP, MAC, RLC PCAP toggles
//     du_high_unit_execution_queues_config — MAC DL queue sizes
//     du_high_unit_expert_execution_config — aggregates queue config
//
//   RLC (Radio Link Control)
//     du_high_unit_rlc_tx_am_config / rx_am_config / am_config — AM mode
//     du_high_unit_rlc_tx_um_config / rx_um_config / um_config — UM mode
//     du_high_unit_rlc_config       — chooses AM or UM per RB
//     du_high_unit_srb_config       — per-SRB RLC config
//
//   F1-U (user-plane interface)
//     du_high_unit_f1u_du_config    — backoff timers for the DU F1-U socket
//
//   Other
//     du_high_unit_triggered_ul_grant_config — triggered UL grant parameters
//     du_high_unit_mac_config                — triggered UL grant per cell
//     du_high_unit_qos_config                — per-5QI DRB RLC + PDCP config
//     du_high_unit_custom_band_config        — custom non-standard NR bands
//
//   Top-level aggregator
//     du_high_unit_config    — all of the above + gNB identity + cells + QoS + SRBs
//     du_high_parsed_config  — wraps du_high_unit_config + a common_cell_cfg baseline
//
//   NOTE: The existing per-field Doxygen comments in this file are comprehensive
//   3GPP-accurate documentation. They are preserved unchanged below. The block
//   above provides the architectural map your professor needs to orient himself
//   before reading the field details.
// =============================================================================
