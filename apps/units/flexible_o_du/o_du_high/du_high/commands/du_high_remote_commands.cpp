...
// =============================================================================
// FILE: commands/du_high_remote_commands.cpp
//
// Purpose (plain English):
//   Implements the execute() methods for the three remote commands declared in
//   du_high_remote_commands.h. Each method:
//     1. Validates the incoming JSON payload field by field, returning an
//        error string on the first problem found (the HTTP layer sends this
//        back to the caller).
//     2. Translates the JSON values into the internal C++ request types.
//     3. Calls configurator.handle_sync_operator_config(req) to apply the
//        change synchronously to the running DU.
//
//   Helper functions shared across commands:
//     parse_cgi           — extracts a NR Cell Global ID (PLMN + NCI).
//     parse_q_hyst_db     — maps a dB integer to the q_hyst_t enum.
//     parse_q_offset_range — maps a dB integer to q_offset_range_t.
//     parse_scs_khz       — maps kHz integer to subcarrier_spacing enum.
//     parse_bounded_int   — generic range-checked integer extractor.
//     find_and_parse_bounded_int — finds a named field then range-checks it.
//     find_and_parse_pci  — extracts a PCI (Physical Cell ID, range 0..1007).
//     parse_sib2/3/4      — parse each SIB's JSON content object.
// =============================================================================
...
// =============================================================================
// ssb_modify_remote_command::execute()
//   JSON: { "cells": [ { "plmn":"...", "nci": N, "ssb_block_power_dbm": P }, ... ] }
//   Range check: ssb_block_power_dbm in [-60, 50].
// =============================================================================
error_type<std::string> ssb_modify_remote_command::execute(const nlohmann::json& json)
{
  ...
  for (const auto& cell : cells_items) {
    // Extract and validate PLMN string.
    ...
    // Extract and validate NCI (NR Cell Identity) as an unsigned integer.
    ...
    // Parse and construct the CGI (Cell Global Identity = PLMN + NCI).
    ...
    // Extract and range-check the new SSB transmit power.
  ...
  // Apply the change synchronously. Returns success=true if applied.
...
// =============================================================================
// rrm_policy_ratio_remote_command::execute()
//   JSON: { "policies": { "resourceType":"PRB", "rRMPolicyMemberList":[...],
//            "min_prb_policy_ratio":N, "max_prb_policy_ratio":N, "dedicated_ratio":N } }
// =============================================================================
error_type<std::string> rrm_policy_ratio_remote_command::execute(const nlohmann::json& json)
{
  ...
  // ---- Resource type (only "PRB" supported for now) -----------------------
  ...
  if (resource_type_key->get<std::string>() == "PRB") {
    ...
    // "RRC" and "DRB" are planned but not yet implemented.
    return make_unexpected("Only 'resourceType' PRB value is supported");
  }

  // RRM Policy members list.

  // ---- Policy member list (PLMN + S-NSSAI per member) --------------------
  ...
  for (const auto& policy_member_key : policy_members_list->items()) {
    ...
    // SST (Slice/Service Type) is mandatory.
    ...
    // SD (Slice Differentiator) is optional within an S-NSSAI.
  ...
  // ---- Optional PRB-share percentages ------------------------------------
  // Each value must be in [0, 100]. All are optional; absent means "no change".
...
// ---- JSON parsing helpers shared between SSB and SIB commands -------------
...
/// Maps a q_hyst_db integer to the q_hyst_t enum. Only even values plus
/// a few specific odd ones are valid (3GPP TS 38.331 Table 6.3.2-1).
...
/// Maps a signed-dB integer to q_offset_range_t. Gaps exist at odd values
/// above ±5 (3GPP TS 38.331 Table 6.3.4-1).
...
/// Maps a kHz integer to subcarrier_spacing enum (15, 30, 60, 120, 240 kHz).
...
/// Generic bounded-integer parser. Returns an error if out of [MIN, MAX].
...
/// Finds a named field and range-checks it in one step.
template <typename T, T MIN, T MAX>
...
/// Finds a PCI field (0..1007) in the given JSON object.
...
/// SIB2 = cell-reselection parameters (q_hyst, serving thresholds, priority).
...
/// SIB3 = intra-frequency neighbour list + excluded-cell list.
static expected<sib3_info, std::string> parse_sib3(const nlohmann::json& content)
{
  ...
  if (excluded_list_it != content.end()) {
    ...
    for (const auto& entry : excluded_list_it->items()) {
      ...
      // pci_range_t::range_t enum values equal the integer widths.
      switch (range_val) {
        case 1:
        case 4:
        case 8:
...
/// SIB4 = inter-frequency carrier list (ARFCN, SSB SCS, reselection thresholds).
static expected<sib4_info, std::string> parse_sib4(const nlohmann::json& content)
{
  ...
  for (const auto& entry : carrier_list_it->items()) {
    ...
    // q_offset_freq is optional (default db0 if absent).
...
// =============================================================================
// sib_update_remote_command::execute()
//   JSON: { "cells": [ { "plmn":"...", "nci":N,
...
error_type<std::string> sib_update_remote_command::execute(const nlohmann::json& json)
{
  ...
  for (const auto& cell : cells_items) {
    ...
    // Parse the SIB object (type + content).
    ...
    // Dispatch to the right parser based on the SIB type string.
