...
// =============================================================================
// FILE: du_high_config_yaml_writer.cpp  (880 lines)
//
// Serialises the full DU High config (all cells, MAC, RLC, scheduler, SIB, PCAP, log, metrics) back to a YAML document. Used by the print-effective-config feature at startup. Mirrors the CLI11 schema one-for-one: for every option registered in the schema there is a corresponding node[key]=value assignment here.
// =============================================================================
...
static YAML::Node build_du_high_pusch_section(const du_high_unit_pusch_config& config)
{
  ...
  node["ema_alpha_cl_pw_control_sinr"] = config.ema_alpha_cl_pw_control_sinr;
...
static YAML::Node build_du_high_pucch_section(const du_high_unit_pucch_config& config)
{
  ...
  node["max_consecutive_kos"]          = config.max_consecutive_kos;
  node["enable_cl_loop_pw_control"]    = config.enable_closed_loop_pw_control;
  node["target_sinr_f0"]               = config.pucch_f0_sinr_target_dB;
  node["target_sinr_f2"]               = config.pucch_f2_sinr_target_dB;
  node["target_sinr_f3"]               = config.pucch_f3_sinr_target_dB;
  node["ema_alpha_cl_pw_control_sinr"] = config.ema_alpha_cl_pw_control_sinr;
...
static YAML::Node build_cell_entry(const du_high_unit_base_cell_config& config)
{
  ...
  node["pci"] = config.pci;
  if (config.sector_id.has_value()) {
    node["sector_id"] = config.sector_id.value();
  }
