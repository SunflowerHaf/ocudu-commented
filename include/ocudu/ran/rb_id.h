...
// =============================================================================
// FILE: include/ocudu/ran/rb_id.h  (176 lines)
//
...
// drb_id_t formatter
template <>
struct formatter<ocudu::drb_id_t> {
  ...
  auto format(ocudu::drb_id_t o, FormatContext& ctx) const
  {
    switch (o) {
...
// srb_id_t formatter
template <>
struct formatter<ocudu::srb_id_t> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(ocudu::srb_id_t o, FormatContext& ctx) const
  {
...
// rb_id_t formatter
template <>
struct formatter<ocudu::rb_id_t> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(ocudu::rb_id_t o, FormatContext& ctx) const
  {
    if (o.is_drb()) {
