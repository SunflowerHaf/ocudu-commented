...
// =============================================================================
// FILE: include/ocudu/adt/circular_map.h  (469 lines)
//
...
struct circular_map_storage<std::vector<T>, false> {
  ...
  {
    ocudu_assert(data.size() == 0 or size == data.size(),
...
public:
  ...
  public:
    ...
    {
      ocudu_assert(idx < ptr->capacity(), "Iterator out-of-bounds ({} >= {})", idx, ptr->capacity());
  ...
  {
    ocudu_assert(it.idx < capacity() and it.ptr == this, "Iterator out-of-bounds ({} >= {})", it.idx, capacity());
    iterator next = it;
    ++next;
