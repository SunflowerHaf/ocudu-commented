...
// =============================================================================
// FILE: include/ocudu/support/executors/task_worker.h  (162 lines)
//
...
public:
  ...
  void push_task_blocking(task_t&& task)
  {
    if (not pending_tasks.push_blocking(std::move(task))) {
      ocudulog::fetch_basic_logger("ALL").debug("Cannot push more tasks into the {} worker queue because it was closed",
                                                t_handle.get_name());
      return;
    }
  }
