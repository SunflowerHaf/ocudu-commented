...
// =============================================================================
// FILE: include/ocudu/phy/upper/channel_state_information_formatters.h  (152 lines)
//
...
/// \brief Custom formatter for \c channel_state_information.
template <>
struct formatter<ocudu::channel_state_information> {
  ...
  template <typename FormatContext>
  auto format(const ocudu::channel_state_information& csi, FormatContext& ctx) const
  {
    ...
    if (helper.is_verbose()) {
      if (csi.sinr_ch_estimator_dB.has_value()) {
        const char* selected =
            (csi.sinr_report_type == ocudu::channel_state_information::sinr_type::channel_estimator) ? "[sel]" : "";
      ...
      if (csi.sinr_post_eq_dB.has_value()) {
        const char* selected =
            (csi.sinr_report_type == ocudu::channel_state_information::sinr_type::post_equalization) ? "[sel]" : "";
      ...
      if (csi.sinr_evm_dB.has_value()) {
        const char* selected =
            (csi.sinr_report_type == ocudu::channel_state_information::sinr_type::evm) ? "[sel]" : "";
      ...
      std::optional<ocudu::phy_time_unit> time_aligment = csi.get_time_alignment();
