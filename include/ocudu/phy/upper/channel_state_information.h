...
// =============================================================================
// FILE: include/ocudu/phy/upper/channel_state_information.h  (296 lines)
//
...
public:
  ...
  static sinr_type sinr_type_from_string(const std::string& sinr_type_str)
  {
    ...
    ocudu_assertion_failure("Invalid SINR type");
    return sinr_type::channel_estimator;
  }
  ...
  void set_symbol_evm(unsigned i_symbol, float evm_)
  {
    ...
    ocudu_assert(i_symbol < symbol_evm.size(), "The OFDM symbol index (i.e., {}) exceeds slot duration.");
  ...
  void set_rsrp_lin(span<const float> rsrp_per_port_lin)
  {
    ...
    ocudu_assert(nof_ports <= MAX_PORTS,
