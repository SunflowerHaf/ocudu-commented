// SPDX-FileCopyrightText: Copyright (C) 2021-2026 Software Radio Systems Limited
// SPDX-License-Identifier: BSD-3-Clause-Open-MPI

// SPDX-FileCopyrightText: Copyright (C) 2021-2026 Software Radio Systems Limited
// SPDX-License-Identifier: BSD-3-Clause-Open-MPI

/**
 * @file radio_uhd_tx_stream.cpp
 * @brief Implementation of UHD transmit streaming: burst management, power ramping, and event handling.
 */

#include "radio_uhd_tx_stream.h"
#include "ocudu/gateways/baseband/buffer/baseband_gateway_buffer_reader_view.h"
#include "ocudu/ocuduvec/zero.h"
#include <uhd/version.hpp>


using namespace ocudu;

/// Receive asynchronous message timeout in seconds.

/// Timeout for a single recv_async_msg() poll call.
static constexpr double RECV_ASYNC_MSG_TIMEOUT_S = 0.001;
/// Transmit timeout in seconds.
/// Timeout for a single transmit (send) call.
static constexpr double TRANSMIT_TIMEOUT_S = 0.001;


/**
 * @brief Polls UHD for one pending asynchronous Tx event message, if any, and dispatches it.
 *
 * Asynchronous Tx messages report what happened to previously-submitted
 * transmissions: a successful burst acknowledgement, a timing error (late),
 * an underflow (USRP ran out of samples mid-burst), a sequence error, or
 * other diagnostic events. This function translates UHD's event codes into
 * the OCUDU radio_event_type enum and forwards relevant events through the
 * notifier and to the burst FSM (so it can transition state appropriately).
 */
void radio_uhd_tx_stream::recv_async_msg()
{
  uhd_exception_handler exception_handler;
  bool                  valid;
  uhd::async_metadata_t async_metadata;

  // Retrieve asynchronous message.

  if (!exception_handler.safe_execution([this, &valid, &async_metadata]() {
        valid = stream->recv_async_msg(async_metadata, RECV_ASYNC_MSG_TIMEOUT_S);
      })) {
    fmt::println(stderr,
                 "Error: receiving asynchronous message for stream {}. {}.",
                 stream_id,
                 exception_handler.get_error_message().c_str());
    return;
  }

  // Skip if it is not valid.

  // No message was waiting within the timeout — nothing to do this poll.
  if (!valid) {
    return;
  }

  // Handle event.

  radio_event_notifier::event_description event_description = {.stream_id = stream_id,
                                                               .channel_id =
                                                                   static_cast<unsigned>(async_metadata.channel),
                                                               .source    = radio_event_source::TRANSMIT,
                                                               .type      = radio_event_type::UNDEFINED,
                                                               .timestamp = std::nullopt};


  switch (async_metadata.event_code) {
    case uhd::async_metadata_t::EVENT_CODE_BURST_ACK:
      // UHD confirms a burst was successfully ended — let the FSM proceed.
      state_fsm.async_event_end_of_burst_ack();
      break;
    case uhd::async_metadata_t::EVENT_CODE_TIME_ERROR:
      // A sample's intended transmission time had already passed — it was dropped.
      event_description.type = radio_event_type::LATE;
      state_fsm.async_event_late_underflow(async_metadata.time_spec);
      break;
    case uhd::async_metadata_t::EVENT_CODE_UNDERFLOW:
    case uhd::async_metadata_t::EVENT_CODE_UNDERFLOW_IN_PACKET:
      // The USRP ran out of samples to transmit mid-burst.
      event_description.type = radio_event_type::UNDERFLOW;
      state_fsm.async_event_late_underflow(async_metadata.time_spec);
      break;
    case uhd::async_metadata_t::EVENT_CODE_SEQ_ERROR:
    case uhd::async_metadata_t::EVENT_CODE_SEQ_ERROR_IN_BURST:
    case uhd::async_metadata_t::EVENT_CODE_USER_PAYLOAD:
      // Lower-priority diagnostic events, reported generically.
      event_description.type = radio_event_type::OTHER;
      break;
#if UHD_VERSION >= 4090000
    case uhd::async_metadata_t::EVENT_CODE_OK:
      // This is not an error. Do nothing.
      // Not an error — no action needed.
      break;
#endif // UHD_VERSION >= 4090000
  }

  // Notify event if it is defined.

  // Only forward the notification if it's a meaningful event type (not UNDEFINED).
  if (event_description.type != radio_event_type::UNDEFINED) {
    notifier.on_radio_rt_event(event_description);
  }
}


/**
 * @brief Re-enqueues recv_async_msg() on the async executor, forming a continuous polling loop.
 *
 * This pattern (a self-rescheduling task) keeps the polling loop alive
 * without blocking a dedicated thread on a busy-wait, and naturally stops
 * when stop_control signals a stop request.
 */
void radio_uhd_tx_stream::run_recv_async_msg()
{
  auto token = stop_control.get_token();
  if (OCUDU_UNLIKELY(token.is_stop_requested())) {
    return;
  }

  // Receive asynchronous message.

  recv_async_msg();

  // Enqueue the task again.

  // Schedule the next poll. report_error_if_not() ensures this never silently
  // fails — if the executor can't accept the task, that's a serious bug.
  report_error_if_not(async_executor.defer([this, tk = std::move(token)]() { run_recv_async_msg(); }),
                      "Unable to run recv async UHD stream task");
}


/**
 * @brief Sends a single block of samples to UHD's tx_streamer.
 *
 * Flattens the multi-channel buffer_reader into the raw pointer array format
 * UHD's API expects, then calls stream->send().
 *
 * @param nof_txd_samples  Output: number of samples actually transmitted.
 * @param buffs            Source sample buffers (one per channel).
 * @param buffer_offset    Sample offset to start from within buffs.
 * @param md               UHD transmit metadata for this block.
 * @return true if no exception was thrown.
 */
bool radio_uhd_tx_stream::transmit_block(unsigned&                             nof_txd_samples,
                                         const baseband_gateway_buffer_reader& buffs,
                                         unsigned                              buffer_offset,
                                         const uhd::tx_metadata_t&             md)
{
  // Extract number of samples.
  unsigned num_samples = buffs[0].size() - buffer_offset;

  // Make sure the number of channels is equal.

  report_fatal_error_if_not(buffs.get_nof_channels() == nof_channels, "Number of channels does not match.");

  // Flatten buffers.

  // UHD's API expects an array of raw void* pointers, one per channel.
  static_vector<void*, RADIO_MAX_NOF_CHANNELS> buffs_flat_ptr;
  for (unsigned channel = 0; channel != nof_channels; ++channel) {
    buffs_flat_ptr.emplace_back(
        const_cast<void*>(reinterpret_cast<const void*>(buffs[channel].subspan(buffer_offset, num_samples).data())));
  }

  // Make UHD buffers.

  uhd::tx_streamer::buffs_type buffs_cpp(buffs_flat_ptr.data(), nof_channels);

  // Safe transmission.

  return safe_execution([this, &buffs_cpp, num_samples, &md, &nof_txd_samples]() {
    nof_txd_samples = stream->send(buffs_cpp, num_samples, md, TRANSMIT_TIMEOUT_S);
  });
}


/**
 * @brief Constructor. Opens the UHD tx_streamer and prepares power ramping resources.
 *
 * @param usrp            The USRP context.
 * @param description     Stream configuration parameters.
 * @param async_executor_ Thread executor for asynchronous Tx message polling.
 * @param notifier_       Receives radio event notifications.
 */
radio_uhd_tx_stream::radio_uhd_tx_stream(uhd::usrp::multi_usrp::sptr& usrp,
                                         const stream_description&    description,
                                         task_executor&               async_executor_,
                                         radio_event_notifier&        notifier_) :
  stream_id(description.id),
  async_executor(async_executor_),
  notifier(notifier_),
  srate_hz(description.srate_hz),
  nof_channels(description.ports.size()),
  discontinuous_tx(description.discontiuous_tx),
  last_tx_timespec(0.0),
  power_ramping_buffer(nof_channels, 0)
{
  ocudu_assert(std::isnormal(srate_hz) && (srate_hz > 0.0), "Invalid sampling rate {}.", srate_hz);

  // Build stream arguments.

  // Build the UHD stream args. CPU-side format is always "sc16" (16-bit
  // signed complex) — this is the in-memory format used throughout the
  // codebase. The over-the-wire (OTW) format, which affects USB/network
  // bandwidth, can differ to trade precision for bandwidth.
  uhd::stream_args_t stream_args = {};
  stream_args.cpu_format         = "sc16";
  switch (description.otw_format) {
    case radio_configuration::over_the_wire_format::DEFAULT:
    case radio_configuration::over_the_wire_format::SC16:
      stream_args.otw_format = "sc16";
      break;
    case radio_configuration::over_the_wire_format::SC12:
      stream_args.otw_format = "sc12";
      break;
    case radio_configuration::over_the_wire_format::SC8:
      stream_args.otw_format = "sc8";
      break;
  }
  stream_args.args     = description.args;
  stream_args.channels = description.ports;


  if (!safe_execution([this, usrp, &stream_args]() {
        stream          = usrp->get_tx_stream(stream_args);
        max_packet_size = stream->get_max_num_samps();
      })) {
    fmt::println("Error: failed to create transmit stream {}. {}.\n", stream_id, get_error_message().c_str());
    return;
  }

  // Use zero padding to absorb power ramping on each new burst.

  // If discontinuous Tx is enabled, prepare the zero-filled power ramping
  // padding buffer, sized to give the RF amplifier the configured ramp time.
  if (discontinuous_tx) {
    power_ramping_nof_samples = description.srate_hz * static_cast<double>(description.power_ramping_us) / 1e6;

    // Align the number of padding samples to the TX buffer size.

    // Align padding length to whole UHD packets — this avoids inefficient
    // partial-packet transmissions for the padding itself.
    power_ramping_nof_samples = (power_ramping_nof_samples / max_packet_size) * max_packet_size;
    double aligned_guard_time = static_cast<double>(power_ramping_nof_samples) * 1000000.0 / srate_hz;
    fmt::print(
        "Aligning TX power ramping guard time to the UHD buffer size, i.e., {}. Aligned guard time: {:.1f} us.\n",
        max_packet_size,
        aligned_guard_time);


    power_ramping_buffer.resize(power_ramping_nof_samples);
    for (unsigned i_channel = 0; i_channel != nof_channels; ++i_channel) {
      ocuduvec::zero(power_ramping_buffer.get_writer()[i_channel]);
    }
  }

  // Notify FSM that it was successfully initialized.

  // Tell the burst FSM the stream is ready to accept transmissions.
  state_fsm.init_successful();
}


/**
 * @brief Transmits a block of samples, handling burst boundaries and power ramping.
 *
 * In continuous Tx mode, this simply forwards every block to UHD with
 * minimal metadata. In discontinuous Tx mode, the burst FSM decides whether
 * this call starts/continues/ends a burst, and if a burst is starting, this
 * function may first transmit a chunk of zero-padding samples to give the
 * amplifier time to ramp up before the real data begins.
 *
 * @param data    The sample buffers to transmit (one per channel).
 * @param tx_md   Metadata: absolute timestamp, and (for discontinuous mode)
 *                optional partial-block start/end markers and emptiness flag.
 */
void radio_uhd_tx_stream::transmit(const baseband_gateway_buffer_reader&        data,
                                   const baseband_gateway_transmitter_metadata& tx_md)
{
  auto token = stop_control.get_token();
  if (OCUDU_UNLIKELY(token.is_stop_requested())) {
    return;
  }


  uhd::tx_metadata_t uhd_metadata;


  bool tx_start_padding = tx_md.tx_start.has_value();
  bool tx_end_padding   = tx_md.tx_end.has_value();


  uhd::time_spec_t time_spec = time_spec.from_ticks(tx_md.ts, srate_hz);
  bool             transmit;
  if (discontinuous_tx) {
    if (tx_start_padding) {
      // Set the timespec to the start of the actual transmission if there is head padding in the buffer.
      // Adjust the timestamp to the start of the actual transmission, not
      // the start of the buffer (which may include leading silence).
      time_spec =
          time_spec.from_ticks(tx_md.ts + static_cast<baseband_gateway_timestamp>(tx_md.tx_start.value()), srate_hz);
    }
    // Update state.
    transmit = state_fsm.on_transmit(uhd_metadata, time_spec, tx_md.is_empty, tx_end_padding);
  } else {
    // Continuous mode: always transmit, no burst start/end logic needed.
    transmit = state_fsm.on_transmit(uhd_metadata, time_spec, false, false);
  }

  // Return if no transmission is required.

  if (!transmit) {
    return;
  }

  // Notify start of burst.

  // ── Start-of-burst handling: notify and apply power ramping padding ──────────
  if (uhd_metadata.start_of_burst) {
    radio_event_notifier::event_description event_description = {.stream_id  = stream_id,
                                                                 .channel_id = 0,
                                                                 .source     = radio_event_source::TRANSMIT,
                                                                 .type       = radio_event_type::START_OF_BURST,
                                                                 .timestamp  = time_spec.to_ticks(srate_hz)};


    notifier.on_radio_rt_event(event_description);

    // Transmit zeros before the actual transmission to absorb power ramping effects.

    if (discontinuous_tx) {
      // Compute the time in number of samples between the end of the last transmission and the start of the current
      // one.
      // Compute the gap since the previous transmission ended.
      unsigned transmission_gap = (time_spec - last_tx_timespec).to_ticks(srate_hz);

      // Make sure that the power ramping padding starts at least 10 microseconds after the last transmission end.

      // Ensure the power ramping padding starts at least 10 microseconds
      // after the previous transmission's end, leaving a clean gap.
      unsigned minimum_gap_before_power_ramping = srate_hz / 100000.0;
      unsigned nof_padding_samples              = 0;
      if (transmission_gap > minimum_gap_before_power_ramping) {
        nof_padding_samples = std::min(power_ramping_nof_samples, transmission_gap - minimum_gap_before_power_ramping);
      }


      // If there's room for padding, transmit zero-valued samples ahead of
      // the real data so the amplifier reaches full power before the actual
      // signal starts (avoiding a distorted leading edge).
      if (nof_padding_samples > 0) {
        unsigned txd_padding_sps_total = 0;


        uhd::tx_metadata_t power_ramping_metadata;
        power_ramping_metadata.has_time_spec  = true;
        power_ramping_metadata.start_of_burst = true;
        power_ramping_metadata.end_of_burst   = false;
        power_ramping_metadata.time_spec = uhd_metadata.time_spec - time_spec.from_ticks(nof_padding_samples, srate_hz);

        // Modify the actual trasnmission metadata, since we have already started the burst with padding.

        // The real data transmission now continues mid-burst (not start-of-burst).
        uhd_metadata.start_of_burst = false;
        uhd_metadata.has_time_spec  = false;
        do {
          unsigned txd_samples = 0;


          baseband_gateway_buffer_reader_view tx_padding =
              baseband_gateway_buffer_reader_view(power_ramping_buffer.get_reader(), 0, nof_padding_samples);


          if (!transmit_block(txd_samples, tx_padding, txd_padding_sps_total, power_ramping_metadata)) {
            fmt::println("Error: failed transmitting power ramping padding. {}.", get_error_message().c_str());
            return;
          }


          power_ramping_metadata.time_spec += uhd::time_spec_t::from_ticks(txd_samples, srate_hz);
          txd_padding_sps_total += txd_samples;


        } while (txd_padding_sps_total < nof_padding_samples);
      }
    }
  }

  // Notify end of burst.

  // ── End-of-burst handling: notify ─────────────────────────────────────────────
  if (uhd_metadata.end_of_burst) {
    radio_event_notifier::event_description event_description = {.stream_id  = stream_id,
                                                                 .channel_id = 0,
                                                                 .source     = radio_event_source::TRANSMIT,
                                                                 .type       = radio_event_type::END_OF_BURST,
                                                                 .timestamp  = time_spec.to_ticks(srate_hz)};
    notifier.on_radio_rt_event(event_description);
  }

  // Determine actual transmission range.

  // ── Determine the actual sample range to transmit ─────────────────────────────
  // In discontinuous mode with partial-block markers, only transmit the
  // portion of the buffer that contains real data (not leading/trailing padding).
  unsigned data_start = discontinuous_tx && tx_start_padding ? tx_md.tx_start.value() : 0;
  unsigned data_nof_samples =
      discontinuous_tx && tx_end_padding ? tx_md.tx_end.value() - data_start : data.get_nof_samples() - data_start;

  // Don't pass the transmission data to UHD if the transmit buffer is empty.

  // If the buffer is flagged empty in discontinuous mode, transmit a
  // zero-length view (effectively a no-op for actual sample data, but the
  // burst flags above still get sent correctly).
  baseband_gateway_buffer_reader_view tx_data =
      discontinuous_tx && tx_md.is_empty ? baseband_gateway_buffer_reader_view(data, 0, 0)
                                         : baseband_gateway_buffer_reader_view(data, data_start, data_nof_samples);


  unsigned nsamples = tx_data.get_nof_samples();

  // Transmit stream in multiple blocks.

  // ── Transmit the actual sample data, possibly across multiple UHD packets ────
  unsigned txd_samples_total = 0;
  do {
    unsigned txd_samples = 0;
    if (!transmit_block(txd_samples, tx_data, txd_samples_total, uhd_metadata)) {
      fmt::println("Error: failed transmitting packet. %s.{}", get_error_message().c_str());
      return;
    }

    // Increment timespec.

    uhd_metadata.time_spec += uhd::time_spec_t::from_ticks(txd_samples, srate_hz);

    // Increment the total amount of received samples.
    txd_samples_total += txd_samples;


  } while (txd_samples_total < nsamples);


  // Remember when this transmission ended, for power ramping gap calculation
  // on the next burst.
  last_tx_timespec = time_spec + uhd::time_spec_t::from_ticks(txd_samples_total, srate_hz);
}


/**
 * @brief Starts the background asynchronous Tx event polling task.
 */
void radio_uhd_tx_stream::start()
{
  stop_control.reset();


  report_error_if_not(async_executor.defer([this, stop_token = stop_control.get_token()]() { run_recv_async_msg(); }),
                      "Unable to run recv async UHD stream task");
}


/**
 * @brief Stops the stream, sending an explicit end-of-burst if currently mid-burst.
 *
 * If the FSM reports the stream was mid-burst when stop() is called (rather
 * than cleanly idle), this sends a final zero-length transmission with
 * end_of_burst set, to leave the USRP's internal state clean for the next
 * time the radio is started.
 */
void radio_uhd_tx_stream::stop()
{
  // Once the stop is complete, no more asynchronous calls and transmissions will take place.
  // Signal the stop control first — no further calls or async polling will proceed.
  stop_control.stop();

  // Send end-of-burst if it is in the middle of a burst.

  if (state_fsm.on_stop()) {
    // Notify end of burst.
    radio_event_notifier::event_description event_description = {.stream_id  = stream_id,
                                                                 .channel_id = 0,
                                                                 .source     = radio_event_source::TRANSMIT,
                                                                 .type       = radio_event_type::END_OF_BURST,
                                                                 .timestamp  = std::nullopt};
    notifier.on_radio_rt_event(event_description);

    // Flatten buffers.

    // Send a minimal (zero-sample) transmission carrying only the
    // end-of-burst flag, to properly close the burst with UHD.
    std::array<cf_t, 4>                          buffer;
    static_vector<void*, RADIO_MAX_NOF_CHANNELS> buffs_flat_ptr;
    for (unsigned channel = 0; channel != nof_channels; ++channel) {
      buffs_flat_ptr.emplace_back(reinterpret_cast<void*>(buffer.data()));
    }

    // Make UHD buffers.

    uhd::tx_streamer::buffs_type buffs_cpp(buffs_flat_ptr.data(), nof_channels);

    // Safe transmission. Ignore return.

    uhd::tx_metadata_t md{};
    md.end_of_burst = true;
    safe_execution([this, &buffs_cpp, &md]() {
      // Actual transmission. Ignore number of transmitted samples.
      // Ignore the result — this is a best-effort cleanup transmission.
      stream->send(buffs_cpp, 0, md, TRANSMIT_TIMEOUT_S);
    });
  }
}
