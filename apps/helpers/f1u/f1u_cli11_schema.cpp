...
// SPDX-License-Identifier: BSD-3-Clause-Open-MPI
// =============================================================================
// FILE: helpers/f1u/f1u_cli11_schema.cpp
//
// Purpose (plain English):
//   Registers F1-U socket configuration with the CLI11 parser. Each F1-U
//   socket can be configured independently via a "--socket" repeatable option
//   that accepts a YAML sub-document specifying bind address, S-NSSAI (sst/sd),
//   5QI, and UDP settings. Multiple --socket entries = multiple sockets, each
//   potentially bound to a different slice or QoS class.
// =============================================================================
...
// Registers per-socket options: bind address, optional S-NSSAI/5QI, UDP params.
...
void ocudu::configure_cli11_f1u_sockets_args(CLI::App& app, f1u_sockets_appconfig& f1u_params)
{
  ...
  // A lambda that parses each --socket entry as its own mini YAML document.
  // This allows repeating --socket multiple times for multiple F1-U sockets.
