...
// =============================================================================
// FILE: du_high_config_cli11_schema.cpp  (2955 lines)
//
// Registers every DU High configuration field (~2900 lines) with the CLI11 YAML/CLI parser. Organised as static helper functions per sub-section (log, pdsch, pusch, pucch, srs, rach, paging, pdcch, sib, csi, scheduler, drx, ssb, tdd, MAC, RLC, QoS, slicing, PCAPs, expert execution, test mode, metrics), each called from the two public entry points: configure_cli11_with_du_high_config_schema() and autoderive_du_high_parameters_after_parsing(). Pattern: each field is registered with a name, default value, description, and optional validator lambda.
// =============================================================================
...
static void configure_cli11_metrics_layers_args(CLI::App& app, du_high_unit_metrics_layer_config& metrics_params)
{
  ...
  add_option(
      app, "--enable_du_proc", metrics_params.enable_du_proc, "Enable DU management and control procedure metrics")
      ->capture_default_str();
}
