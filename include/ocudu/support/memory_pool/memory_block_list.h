...
// =============================================================================
// FILE: include/ocudu/support/memory_pool/memory_block_list.h  (223 lines)
//
...
public:
  ...
  {
    ocudu_assert(is_aligned(block, min_memory_block_align()), "The provided memory block is not aligned");
  ...
  void* pop() noexcept
  {
    ocudu_assert(not empty(), "pop() called on empty list");
    node* last_head = head;
    head            = head->next;
    if (head == nullptr) {
      tail = nullptr;
  ...
  {
    ocudu_assert(not other.empty(), "Trying to steal from empty memory_block list");
