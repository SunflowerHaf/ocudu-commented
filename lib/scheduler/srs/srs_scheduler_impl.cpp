...
// =============================================================================
// FILE: lib/scheduler/srs/srs_scheduler_impl.cpp  (518 lines)
//
...
{
  if (ue_cfg.init_bwp().ul.ded() == nullptr or not ue_cfg.init_bwp().ul.ded()->srs_cfg.has_value()) {
    return;
  }
...
void srs_scheduler_impl::reconf_ue(const ue_cell_configuration& new_ue_cfg, const ue_cell_configuration& old_ue_cfg)
{
  if (new_ue_cfg.init_bwp().ul.ded() != nullptr and old_ue_cfg.init_bwp().ul.ded() != nullptr and
      new_ue_cfg.init_bwp().ul.ded()->srs_cfg.has_value() and old_ue_cfg.init_bwp().ul.ded()->srs_cfg.has_value()) {
    // Both old and new UE config have SRS config.
    // Both old and new UE config have SRS config.
    const auto& new_srs_cfg = new_ue_cfg.init_bwp().ul.ded()->srs_cfg.value();
    const auto& old_srs_cfg = old_ue_cfg.init_bwp().ul.ded()->srs_cfg.value();
