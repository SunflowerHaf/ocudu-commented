...
// =============================================================================
// FILE: services/worker_manager/worker_manager.cpp  (679 lines)
//
// The main thread-pool factory (679 lines). Creates all the task executors used by the split-8 gNB: the main pool (general-purpose tasks), the DU-Low upper-PHY executors (separate pools per cell for PDSCH/PUSCH/PUCCH), the DU-High executors (MAC scheduling, RLC, F1AP, PDCP, UE management), CU-CP and CU-UP executors, and the RU-SDR baseband Tx/Rx executors. CPU affinity masks from the config are applied to each pool.
// =============================================================================
