...
// =============================================================================
// FILE: du_low_config_yaml_writer.cpp
//
// Purpose (plain English):
//   Serialises the parsed DU-Low config back to a YAML document — used by the
//   "print effective config" feature that dumps every value (including
//   defaults) at startup, so the user can capture exactly what was in effect
//   for the run. Straight mirror of the CLI11 schema: for every field
//   registered as a command-line/YAML option, there is a corresponding
//   node[...] = value assignment here.
// =============================================================================
...
static void fill_du_low_log_section(YAML::Node node, const du_low_unit_logger_config& config)
{
  ...
  // Only emit optional keys when they were actually set.
...
// ---- expert_execution section (threads + PDSCH processor type) ------------
...
// ---- expert_phy section (all the algorithm knobs) --------------------------
...
// ---- HAL / BBDEV sub-sections ---------------------------------------------
// Only emitted if HAL is configured. Not used in the software (UHD) build path.
...
{
  // Common metrics block (log/JSON toggles etc.) is shared with the DU High.
...
// Attaches each sub-section under its expected key in the parent YAML node.
