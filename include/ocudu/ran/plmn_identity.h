...
// =============================================================================
// FILE: include/ocudu/ran/plmn_identity.h  (274 lines)
//
...
template <>
struct hash<ocudu::plmn_identity> {
  size_t operator()(const ocudu::plmn_identity& s) const noexcept { return hash<std::string>{}(s.to_string()); }
};
...
template <>
struct formatter<ocudu::mobile_country_code> {
  ...
  auto format(const ocudu::mobile_country_code& val, FormatContext& ctx) const
  {
...
template <>
struct formatter<ocudu::mobile_network_code> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const ocudu::mobile_network_code& val, FormatContext& ctx) const
  {
    return format_to(ctx.out(), "{}", val.to_string());
  }
};
...
template <>
struct formatter<ocudu::plmn_identity> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const ocudu::plmn_identity& val, FormatContext& ctx) const
  {
    return format_to(ctx.out(), "{}", val.to_string());
  }
};
