...
// =============================================================================
// FILE: include/ocudu/support/memory_pool/ring_buffer_pool.h  (135 lines)
//
...
private:
  struct overflow_checker {
    ...
    {
      ocudu_assert(first_tick or tic_index != last_tick_index,
                   "This function cannot be called multiple times for the same tick");
      if (OCUDU_UNLIKELY(first_tick)) {
        // first tick call.
        // first tick call.
        last_tick_index = tic_index - 1;
        first_tick      = false;
      } else if (OCUDU_UNLIKELY(last_tick_index > tic_index)) {
        // wrap-around case.
        // wrap-around case.
...
private:
  using overflow_detector_t =
      std::conditional_t<OCUDU_IS_DEFINED(ASSERTS_ENABLED), overflow_checker, no_overflow_checker>;
