...
// =============================================================================
// FILE: include/ocudu/support/executors/priority_task_worker.h  (139 lines)
//
...
template <>
struct formatter<ocudu::task_priority> {
  ...
  auto format(const ocudu::task_priority& prio, FormatContext& ctx) const
  {
    fmt::format_to(ctx.out(), "max");
    if (prio != ocudu::task_priority::max) {
      fmt::format_to(ctx.out(), "-{}", static_cast<size_t>(ocudu::task_priority::max) - static_cast<size_t>(prio));
    }
    return ctx.out();
  }
};
