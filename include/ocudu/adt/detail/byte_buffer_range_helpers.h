...
// =============================================================================
// FILE: include/ocudu/adt/detail/byte_buffer_range_helpers.h  (97 lines)
//
...
bool compare_byte_buffer_range(const LhsContainer& lhs, const RhsContainer& rhs)
{
  ...
  if constexpr (std::is_convertible_v<RhsContainer, span<const uint8_t>>) {
    ...
    ocudu_sanity_check(other_it == rhs.end(), "segment list is in invalid state");
    return true;
  }
  ...
  if constexpr (is_byte_buffer_range<RhsContainer>::value) {
    ...
    ocudu_sanity_check(seg_it1 == segments1.end() and seg_it2 == segments2.end(), "byte buffers are in invalid state");
    return true;
  }
