...
// =============================================================================
// FILE: lib/fapi_adaptor/phy/phy_fapi_fastpath_adaptor_impl.cpp  (26 lines)
//
...
phy_fapi_fastpath_adaptor_impl::phy_fapi_fastpath_adaptor_impl(
    std::vector<std::unique_ptr<phy_fapi_sector_fastpath_adaptor>> sector_adaptors_) :
  sector_adaptors(std::move(sector_adaptors_))
{
  ocudu_assert(!sector_adaptors.empty(), "Cannot create a PHY-FAPI adaptor with zero cells");
}


phy_fapi_sector_fastpath_adaptor& phy_fapi_fastpath_adaptor_impl::get_sector_adaptor(unsigned cell_id)
{
