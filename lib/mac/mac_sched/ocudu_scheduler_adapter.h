...
// =============================================================================
// FILE: lib/mac/mac_sched/ocudu_scheduler_adapter.h  (171 lines)
//
// LIBRARY: lib/mac
...
/// \brief This class adapts OCUDU scheduler interface to operate with OCUDU MAC.
...
/// rather than being processed inline. We defer the processing because we do not want it to take place while
/// the scheduler is still processing the slot_indication, given that latter has higher priority.
class ocudu_scheduler_adapter final : public mac_scheduler_adapter
{
public:
  explicit ocudu_scheduler_adapter(const ocudu_mac_sched_config& params, rnti_manager& rnti_mng_);
...
private:
  ...
  {
  public:
    cell_handler(ocudu_scheduler_adapter& parent_, const sched_cell_configuration_request_message& sched_cfg);
  ...
  private:
    ...
    ocudu_scheduler_adapter& parent;
  ...
  {
  public:
    explicit sched_config_notif_adapter(ocudu_scheduler_adapter& parent_) : parent(parent_) {}
