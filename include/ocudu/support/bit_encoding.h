...
// =============================================================================
// FILE: include/ocudu/support/bit_encoding.h  (161 lines)
//
...
/// \brief Custom formatter for bit_encoder
template <>
struct formatter<ocudu::bit_encoder> {
  ...
  auto format(const ocudu::bit_encoder& s, FormatContext& ctx) const
  {
...
/// \brief Custom formatter for bit_encoder
template <>
struct formatter<ocudu::bit_decoder> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const ocudu::bit_decoder& s, FormatContext& ctx) const
  {
    if (s.next_bit_offset() == 0) {
