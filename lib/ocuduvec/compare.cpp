...
// =============================================================================
// FILE: lib/ocuduvec/compare.cpp  (241 lines)
//
...
const char* ocudu::ocuduvec::detail::find(span<const char> input, const char* value)
{
  // Input index.
  // Input index.
  unsigned index = 0;
...
std::pair<unsigned, float> ocudu::ocuduvec::max_abs_element(span<const cf_t> x)
{
  unsigned i         = 0;
  unsigned len       = x.size();
  ...
  // Prepare range of indexes in SIMD register.
  alignas(SIMD_BYTE_ALIGN) std::array<int32_t, OCUDU_SIMD_CF_SIZE> simd_vector_max_indexes;
  ...
  simd_i_t simd_inc         = ocudu_simd_i_set1(OCUDU_SIMD_CF_SIZE);
  simd_i_t simd_max_indexes = ocudu_simd_i_set1(0);


  simd_f_t simd_max_values = ocudu_simd_f_set1(-INFINITY);
  ...
  for (unsigned simd_end = OCUDU_SIMD_CF_SIZE * (len / OCUDU_SIMD_CF_SIZE); i != simd_end; i += OCUDU_SIMD_CF_SIZE) {
    ...
    // Load 2 SIMD words of floats.
    simd_f_t simd_x1 = ocudu_simd_f_loadu(reinterpret_cast<const float*>(x.data() + i));
    simd_f_t simd_x2 = ocudu_simd_f_loadu(reinterpret_cast<const float*>(x.data() + i + OCUDU_SIMD_CF_SIZE / 2));

    // Calculates the squares.

    // Calculates the squares.
    simd_f_t simd_mul1 = ocudu_simd_f_mul(simd_x1, simd_x1);
    simd_f_t simd_mul2 = ocudu_simd_f_mul(simd_x2, simd_x2);
    ...
    // Get SIMD selector mask.
    simd_sel_t res = ocudu_simd_f_max(simd_abs2, simd_max_values);

    // Select the indexes and values for the maximum.

    // Select the indexes and values for the maximum.
    simd_max_indexes = ocudu_simd_i_select(simd_max_indexes, simd_indexes, res);
    simd_max_values  = ocudu_simd_f_select(simd_max_values, simd_abs2, res);
  ...
  // Store the indexes and values from SIMD registers into arrays.
  alignas(SIMD_BYTE_ALIGN) std::array<float, OCUDU_SIMD_I_SIZE> simd_vector_max_values;
  ocudu_simd_i_store(simd_vector_max_indexes.data(), simd_max_indexes);
  ocudu_simd_f_store(simd_vector_max_values.data(), simd_max_values);
...
std::pair<unsigned, float> ocudu::ocuduvec::max_element(span<const float> x)
{
  ...
  for (unsigned simd_end = OCUDU_SIMD_CF_SIZE * (len / OCUDU_SIMD_CF_SIZE); i != simd_end; i += OCUDU_SIMD_CF_SIZE) {
    ...
    // Load SIMD word of floats.
    simd_f_t simd_x = ocudu_simd_f_loadu(reinterpret_cast<const float*>(x.data() + i));

    // Get SIMD selector mask.

    // Get SIMD selector mask.
    simd_sel_t res = ocudu_simd_f_max(simd_x, simd_max_values);

    // Select the indexes and values for the maximum.

    // Select the indexes and values for the maximum.
    simd_max_indexes = ocudu_simd_i_select(simd_max_indexes, simd_indexes, res);
    simd_max_values  = ocudu_simd_f_select(simd_max_values, simd_x, res);
...
unsigned ocudu::ocuduvec::count_if_part_abs_greater_than(span<const ocudu::cf_t> x, float threshold)
{
  unsigned count = 0;
