...
// =============================================================================
// FILE: metrics/du_high_rlc_metrics_consumers.cpp
//
// Purpose (plain English):
//   Implements the three RLC metric consumers declared in the header.
//
//   JSON: generates a JSON document from the rlc_metrics using the shared
//         json_generators helper (output mimics the Open Fronthaul JSON schema).
//
//   Log:  formats a compact log line: "RLC Metrics: du=N ue=N rb=N TX=[...] RX=[...]"
//         where TX/RX are formatted by the core RLC metric formatter.
//
//   E2:   simply forwards the raw rlc_metrics to the notifier interface —
//         the E2 agent serialises it into an ASN.1 RIC Indication message.
// =============================================================================
...
void rlc_metrics_consumer_json::handle_metric(const app_services::metrics_set& metric)
{
  ...
  // Serialise to JSON string and push to the HTTP gateway.
...
{
  // Forward the raw data to the E2 agent; it will ASN.1-encode it.
