...
// =============================================================================
// FILE: include/ocudu/support/async/event_sender_receiver.h  (234 lines)
//
...
public:
  ...
  void set(Result&& data)
  {
    ...
    ocudu_assert(receiver != nullptr, "Sender not connected to receiver");
...
public:
  ...
  ~event_receiver()
  {
    ocudu_assert(state != state_t::unset, "Event receiver destroyed without being set or cancelled");
  }


  event_sender<Data> get_sender()
  {
    ocudu_assert(state == state_t::uninit, "Only one sender can be connected to an event receiver");
    state = state_t::unset;
  ...
  {
    ocudu_assert(successful(), "The event result has not be set");
    return data.value();
  }
...
private:
  ...
  {
    ocudu_assert(state == state_t::unset, "The event cannot be set multiple times");
    state      = state_t::set;
    this->data = result;
